 ...
 * edge: CALLED_BY all API route handlers in apps/studio
 * edge: COMPLEMENTS apps/content-os/src/lib/api-handler.ts (identical contract, separate deployment unit)
 * prompt: Verify userId is never empty in routes that require auth; test Zod parse error path returns 400 without schema details; test 5xx sanitization hides error internals
 */
